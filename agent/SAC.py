...
def weight_init(m):
    #Custom weight initialization:
        # Linear layers: Orthogonal initialization
        # Conv layers: Delta-orthogonal initialization (useful for deep networks)
...
def gaussian_logprob(noise, log_std): # Computes the log probability of Gaussian noise, used for calculating policy entropy in SAC.
    """Compute Gaussian log probability."""
    residual = (-0.5 * noise.pow(2) - log_std).sum(-1, keepdim=True)
    return residual - 0.5 * np.log(2 * np.pi) * noise.size(-1)


def squash(mu, pi, log_pi): #Applies the tanh squashing function to bound actions to [-1, 1]
...
class Actor(nn.Module):
    ...
    def forward(
        ...
        mu, log_std = self.fc(obs).chunk(2, dim=-1)
